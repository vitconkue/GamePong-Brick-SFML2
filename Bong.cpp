...
void Bong::CapNhat(RenderWindow* window)
{
	...
	{
		this->VanToc.x *= -1.1; //Vận tốc theo x sẽ tăng thêm 10%
		//Xử lý theo tính chất của va chạm vật lý 
		if (this->VanToc.y > 0)
		{
			if (this->player1->VanToc.y > 0)
			{
				this->VanToc.y = this->VanToc.y * 1.1; //Vận tốc theo y cũng sẽ tăng thêm 10%
			}
			else if (this->player1->VanToc.y < 0)
			{
				this->VanToc.y = this->VanToc.y * -1.1;
			}
		}
		else if (this->VanToc.y < 0)
		{
			if (this->player1->VanToc.y > 0)
			{
				this->VanToc.y = this->VanToc.y * -1.1;
	...
	if (this->KiemTraVaCham(this->player2))
	{
		...
		if (this->VanToc.y > 0)
		{
			if (this->player2->VanToc.y > 0)
			{
				this->VanToc.y = this->VanToc.y * 1.1;//Vận tốc theo y cũng sẽ tăng thêm 10%
			}
			else if (this->player2->VanToc.y < 0)
			{
				this->VanToc.y = this->VanToc.y * -1.1;
			}
		}
	...
	{
		this->VanToc.y *= -1; //nó sẽ đập ngược lại theo tính chất vật lý
		this->sound->play();
	}
...
void Bong::Reset(sf::RenderWindow* window)
{
	...
	this->VanToc.y = ((rand() % 2) == 0) ? 1.2 : -1.2; //Random hướng của bóng theo y với vận tốc 1.2f
